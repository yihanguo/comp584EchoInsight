...
class MainLayerAgent:
    ...
    def suggest_features(self, review: ReviewRecord, feature_catalog: list[FeatureDefinition]) -> list[dict]:
        review_counts = token_counts(review.combined_text)
        title_counts = token_counts(review.title)
        suggestions = []
        ...
        for feature in feature_catalog:
            ...
            title_similarity = cosine_from_counters(title_counts, feature.prototype_counts)
            phrase_overlap = len(phrase_hits(review.combined_text, feature.positive_keywords + feature.negative_keywords))
    ...
    def _rating_alignment(self, review: ReviewRecord) -> float:
        counts = token_counts(review.combined_text)
...
class ValidationLayer:
    ...
    def validate(self, review: ReviewRecord, classifications: list[dict]) -> dict:
        ...
        review_length = max(len(token_counts(review.combined_text)), 1)
